...
def load_demo_transactions() -> List[Dict[str, Any]]:
    """
    Helper function to load synthetic transactions for development/testing.
    """
...
def get_worker_financial_summary(
    ...
    """
    Enforces worker data isolation by operating strictly on transactions belonging to worker_id.
    Executes analyze_transactions and returns typed FinancialAnalyticsResult.
    """
    raw_records = transactions_override if transactions_override is not None else load_demo_transactions()

    # STRICT WORKER DATA ISOLATION: filter transactions belonging strictly to worker_id
    worker_transactions = [
