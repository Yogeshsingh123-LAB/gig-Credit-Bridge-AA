...
import { 
  LayoutDashboard, Layers, ArrowLeftRight, CheckCircle2, 
  Award, FileText, Share2, UserCheck, LogOut, ShieldCheck,
  Menu, X 
} from 'lucide-react';
...
export const WorkerLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { to: '/worker/platforms', label: 'Gig Platforms', icon: Layers },
    { to: '/worker/transactions', label: 'Transactions', icon: ArrowLeftRight },
    { to: '/worker/verification', label: 'Verification', icon: CheckCircle2 },
    { to: '/worker/score', label: 'Readiness Score', icon: Award },
    { to: '/worker/passport', label: 'Credit Passport', icon: FileText },
    { to: '/worker/consent', label: 'Consent Sharing', icon: Share2 },
    { to: '/worker/profile', label: 'My Profile', icon: UserCheck },
  ];
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                      isActive
                        ? 'bg-emerald-600/20 text-emerald-400 border-l-4 border-emerald-500 pl-2'
